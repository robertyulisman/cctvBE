...
exports.UserLogin = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign(
      {
        exp: Math.floor(
          new Date(userLogin.customerProfile.subscribeExpired).getTime() / 1000
        ),
        id: userLogin.id,
...
exports.muteConfig = async (req, res, next) => {
  try {
    const { registration_token, mute_notif } = req.body;
    const updateMuteConfig = await Devices.update(
      { mute_notif: mute_notif },
      {
        where: {
    ...
    if (updateMuteConfig) {
      res.status(200).send({
        success: true,
        msg: "Success update mute config",
      });
    } else {
      res.status(404).send({
        success: false,
        msg: "Failed update mute config",
      });
    }
  } catch (err) {
    console.log(err);
